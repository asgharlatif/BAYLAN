...
//alert(document.location.href.indexOf('news'));
if(document.location.href.indexOf('news') != -1)
  pauseLength = 5000;   //alternate page scroller
else
  pauseLength = 5000;  //home page scroller
  


//the above allows two differnet scrollers to scroll at different speeds
...
//	this.pauseLength = 5000     - this line needs to be activated to have two scrollers (on different pages) affected by the same variable
	this.inc = 2
	this.speed = 30
	this.fromX = 0
	this.fromY = this.h
...
function NewsTickerBuild() {
	...
	this.div = '<div id="'+this.name+'" style="overflow:hidden;">' // overflow:hidden is used for clipping in Opera
    ...
    seed = randomNum(newsItemA + 1); // set the amount of news items to rotate in opera
	this.div += '</div>'
}
function NewsTickerActivate(autostart) {
	...
	if (autostart!=false) setTimeout(this.obj+'.start()',pauseLength)
    // pauseLength needs to be changed to this.pauseLength to have the two scrollers affected by the same variable
}
...
function NewsTickerSlide(obj1,obj2) {
	...
	if ((this.dx!=0 && Math.floor(obj2.x)==0) || (this.dy!=0 && Math.floor(obj2.y)==0)) {
		...
		this.timer = setTimeout(this.obj+'.started=false;'+this.obj+'.start()',pauseLength)  
    // pauseLength needs to be changed to this.pauseLength to have the two scrollers affected by the same variable
	}
}
NewsTicker.count = 0