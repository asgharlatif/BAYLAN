...
msql="delete from  basketItems where basketItemID="&trim(vch)&""
conn.Execute(msql)
next
end if
...
if qt(j)=0 then
  Response.Write "<div align=center> <b><font face=arial size=2 color=red> >>>>  Qty can not be a ZERO plus replace ZERO with valid QTY.</font></b></div>"
	...
	msql="update basketItems set quantity="&qt(j)&" where basketItemID="&xbid&""
	conn.Execute(msql)
	'Response.Write msql
	xbid=mbid+j
end if	
...
if request("more") <> "" then  'More Shopping pressed
	SQLtxt3="SELECT SUM(Quantity * priceEach)AS tot, COUNT(*) AS nitems FROM vCart  where basketid='"&cartid&"'"
	RScartitems.CursorLocation = adUseClient
	RScartitems.Open SQLtxt3,conn,adOpenForwardOnly,adLockReadOnly,adcmdtext
	zafar=mcd&"-"&mscat&"-"&RScartitems("tot")&"-"&RScartitems("nitems")&"-"&cartid
...
if (j != 1) {
window.location.href="checkout.asp?bid="+document.myform.cartidt.value;
}
      ...
      <TABLE border=0 cellPadding=0 cellSpacing=0 width=650>
        ...
        <TR> 
          <TD height=290 vAlign=top><BR>
            ...
            <BR>
            <FORM action="cart.asp?mact=rec&mbid=<%=mbid%>&cartid=<%=cartID%>&trec=<%=trec%>&mcd=<%=mcd%>" method="post" name="myform">
              <input type="hidden" name="cartidt" value =<%=cartid%>>
