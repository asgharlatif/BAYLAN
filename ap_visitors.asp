...
RsF.Open "select * from avisitors where email='" & trim(email) & "'",conn,adOpenForwardOnly,adLockReadOnly,adcmdtext
set RsF.ActiveConnection=nothing
	if rsf.EOF <> true  then
 if  (subt)="sc" then

if (rsf("myaction"))="nc" then
Conn.Execute("update avisitors set myaction='sc' where email='" & trim(email) & "'")
...
Conn.Execute("update avisitors set myaction='nc' where email='" & trim(email) & "'")
...
Response.Write "<p align='center'> <font face='verdana,arial' color='red'>Already Unsubscribe Found.</font></p>"
	...
	if rsf.EOF <> false  then	
		...
		Conn.Execute(sqls)
'response.write sqls
' mb = "<IMG SRC=""http://www.arshi.com.pk/images/logobbl3.gif""<BR><br><BR><BR>" & chr(13) &chr(10)
...
mb=mb & "<BR><br>Baylan Technology Inc. is a computer-based enterprise with resolute commitment to provide best solution to Computer and Network Users. Our dedicated efforts have enabled us to achieve Dealer/Reseller status in Karachi." 
mb=mb & "<br><br>We also welcome you most cordially if you desire to subscribe as a  Reseller / Member for our products sale / use , which all carry BAYLAN (Networking, Communication & Structured Cabling Products) and CELL POWER (Intelligent UPS+AVR+LSP+CIP),Metrologic, Promag, Commscope. Netche. We also keep stock of various computer related products."
...
'objMessage.Host =rsemail("ip")
objMessage.From = "Baylan Technology Inc. (Subscribe)" & rsemail("subscribe")  'From 
'objMessage.FromName = "ARSHI COMPUTER (Subscribe)"
