...
<link rel="stylesheet" href="/aadmin/arshilogin.css" type="text/css">
</head>
<%
...
if rsUserAdd.EOF then
	sameAddress="same"
	regsql2="insert into AshippingAddress values('"& mchkid &"','0','"& rsUser("Email") &"','"& rsuser("Add1") &"','" & rsUser("Add2") &"','"& rsUser("city") &"','" &rsUser("ph") &"','"& rsuser("fax") &"','"& rsuser("PostalCode") &"','"& rsUser("country") &"')"
...
<%if checkid = "" then%>
<form action="checkoutID.asp?bid=<%=request("bid")%>" method="post" onSubmit="return Form1_Validator(this)" name="Form1">
...
<%else%>
<br>
<form action="cartCheckout.asp?cartID=<%=bid2%>&customerID=<%=mChkid%>&shipID=<%=sShipAdd%>&ctype=<%="Member"%>" method=Post>
...
<br>
<form action="cartCheckout.asp?cartID=<%=request("bid")%>&dealerID=<%=mChkid%>&shipID=<%=sShipAdd%>&ctype=<%="Dealer"%>" method="post">
