...
session("bids")=Request("bids")
	...
	SQLtxt="select * from vcart where basketid="&session("bids")&"" 
...
window.location.href="cartCheckout.asp?xFreight="+aa;
}
...
{
alert ('aw')
window.location.href="cartCheckout.asp"
}
...
%>
            <FORM action="cartCheckout.asp?cartid=<%=session("cartID")%>&custID=<%=session("CustomerID")%>&DealID=<%=session("dealerID")%>" method=post onSubmit="return myform_Validator(this)" name="myform">
         ...
         if trim(aFreight) <> "" then
          ...
          %>
              ...
              <%
				...
				Response.Redirect "process_cartCheckout.asp?cartid=" & session("cartID") & "&custID=" & session("CustomerID") & "&DealID=" & session("dealerID")
				%>
              <script language="javascript">
                alert ("i am checkout")
                </script>
