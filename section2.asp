...
Stxt="SELECT   * FROM  mainopt WHERE     msection = 'sectionB'"
...
SQLtxt="select * from Subcat where catcode='"&mSec&"' " 
...
<%
sqltxt2="select *,[dbo].EditPrice (retail * " & price &" ) as retail,[dbo].EditPrice (pga * " & price &" ) as pga  from vPrice  where showmp= 'Yes' and category='"& rsscat("category")& "'" 
...
%>
  <tr> 
    <td width="66%" class="text4"><a href="page2.asp?Mdata=<%=rsvprice("catcode")%>&head1=<%=rsvprice("groupdesc")%>&head2=<%=rsvprice("category")%>&mcode=<%=rsvprice("CatId")%>"><%=rsVprice("item")%></a></td>
    <td width="34%" align="right" class="text4" ><%=formatnumber(rsVprice("pga"),2)%></td>
  </tr>
<%
