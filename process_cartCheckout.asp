...
if Dealerid <> "" then
	...
	SQLtxt="select * from vcart where basketid='"&cartID&"'" 
  ...
  'on error resume next
	...
	st="insert into orders ( dealerid,shippingaddressid,taxrate,taxCharged, "
...
'Response.Write st
	...
	sqlmax="SELECT  MAX(orderid) AS Expr1 FROM orders"
...
sqltxt2="insert into orderDetail values("&mlast&", "&rscart("productid")&","&rscart("quantity")&","&rscart("priceEach")&","&ntot&",'"&rscart("taxable")&"','"&rscart("freeshipping")&"',"&rscart("weight")&")"
...
Setxt="select * from Aextras" 
	...
	<div align="center">
	<p>&nbsp;</p>
	<p><img src="aadmin/images/spacer.gif" width="500" height="1"><br>
    ...
    <img src="aadmin/images/spacer.gif" width="500" height="1"> 
    <br>
    <span class="text">|</span> <a href ="network.asp" class="links1">HOME</a> <span class="text">|</span></p>
</div>
...
sub sendTOclient()
	...
	SQLtxt3="select * from vorders where orderid='"&mlast&"'" 
    ...
    conn.Execute("delete from basket where basketid="&trim(Request.QueryString("cartid"))&"")
    conn.Execute("delete from basketitems where basketid="&trim(Request.QueryString("cartid"))&"")
...
strHTML = strHTML & "<table width=650 border=0 cellpadding=0>"
...
'objMessage.Addcc  rsemail("toemail")     'cc to bbl
